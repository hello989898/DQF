...
def build_qua_negatives(anc_idxs, pos_idxs, similarities, neg_imgs_idx, id_to_classid, alpha_value, traces_n1, num_retries=50):
    # If no similarities were computed, return a random negative
    final_neg = []
    if similarities is None:
        if traces_n1 is None:
            for i in range(len(anc_idxs)):
                selid=random.sample(neg_imgs_idx,1)
                selclass=id_to_classid[selid[0]]
                ancclass=id_to_classid[anc_idxs[i]]
                while selclass==ancclass:
                    selid=random.sample(neg_imgs_idx,1)
                    selclass=id_to_classid[selid[0]]
                final_neg.append(selid[0])
            return final_neg
        else:
            for i in range(len(anc_idxs)):
                selid=random.sample(neg_imgs_idx,1)
                selclass=id_to_classid[selid[0]]
                ancclass=id_to_classid[anc_idxs[i]]
                corresponding_n1_class=id_to_classid[traces_n1[i]]
                while (selclass==ancclass) or (selclass==corresponding_n1_class):
                    selid=random.sample(neg_imgs_idx,1)
                    selclass=id_to_classid[selid[0]]
                final_neg.append(selid[0])
            return final_neg
    ...
    for (anc_idx, pos_idx) in zip(anc_idxs, pos_idxs):
        ...
        for iteration in range(num_retries):
            ...
            if id_to_classid[idx_neg] != anchor_class:
                if traces_n1 is not None:
                    if idx_neg in traces_n1:
                        continue
...
class SemiHardTripletGenerator():
    ...
    def __getitem__(self,index):
        ...
        traces_n1 = build_qua_negatives(traces_a, traces_p, self.similarities, self.neg_traces_idx, self.id_to_classid, self.alpha_value, None)
        traces_n2 = build_qua_negatives(traces_a, traces_p, self.similarities, self.neg_traces_idx, self.id_to_classid, self.alpha_value, traces_n1)

        return self.traces[traces_a], self.traces[traces_p], self.traces[traces_n1], self.traces[traces_n2]
...
class BatchTestDataGenerator():
    def __init__(self, Test_Data_PATH, num_shot, num_query=15, num_way=100):
...
def get_test_dataset(test_path, num_shot, num_query, num_way):
    return BatchTestDataGenerator(test_path, num_shot, num_query, num_way)
